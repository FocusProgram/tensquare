package com.tensquare.qa.dao;

import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.JpaSpecificationExecutor;

import com.tensquare.qa.pojo.Problem;
import org.springframework.data.jpa.repository.Query;


/**
 * 数据访问接口
 * @author Administrator
 *
 */
public interface ProblemDao extends JpaRepository<Problem,String>,JpaSpecificationExecutor<Problem>{

    /**
     * 最新问答列表
     * @param labelid
     * @param pageable
     * @return
     */
    @Query(value ="SELECT * FROM tb_problem,tb_pl WHERE id = problemid AND labelid=?1 ORDER BY replytime DESC",nativeQuery = true)
    public Page<Problem> newList(String labelid, Pageable pageable);

    /**
     * 热门问答列表
     * @param labelid
     * @param pageable
     * @return
     */
    @Query(value ="SELECT * FROM tb_problem,tb_pl WHERE id = problemid AND labelid=?1 ORDER BY reply DESC",nativeQuery = true)
    public Page<Problem> hostList(String labelid, Pageable pageable);


    /**
     * 等待问答列表
     * @param labelid
     * @param pageable
     * @return
     */
    @Query(value ="SELECT * FROM tb_problem,tb_pl WHERE id = problemid AND labelid=?1 AND solve=0 ORDER BY reply DESC",nativeQuery = true)
    public Page<Problem> waitList(String labelid, Pageable pageable);

    /**
     * 最新首页问答列表
     * @param labelid
     * @param pageable
     * @return
     */
    @Query(value ="SELECT * FROM tb_problem,tb_pl WHERE id = problemid ORDER BY replytime DESC",nativeQuery = true)
    public Page<Problem> homeNewList(Pageable pageable);

    /**
     * 热门首页问答列表
     * @param labelid
     * @param pageable
     * @return
     */
    @Query(value ="SELECT * FROM tb_problem,tb_pl WHERE id = problemid ORDER BY reply DESC",nativeQuery = true)
    public Page<Problem> homeHostList(Pageable pageable);


    /**
     * 等待首页问答列表
     * @param labelid
     * @param pageable
     * @return
     */
    @Query(value ="SELECT * FROM tb_problem,tb_pl WHERE id = problemid AND solve=0 ORDER BY reply DESC",nativeQuery = true)
    public Page<Problem> homeWaitList(Pageable pageable);

	
}
